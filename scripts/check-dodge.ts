...
assert.ok(end.warningSeconds >= 0.42, '가장 어려울 때도 예고 시간이 있어야 합니다.');
...
assert.equal(makeDodgeObstacle(4, 6, 1234).kind, 'block', '중반부터는 다른 모양의 장애물도 섞여야 합니다.');
...
assert.ok(spawnCount >= 25, '후반에는 피하기 어려울 만큼 장애물이 충분히 늘어나야 합니다.');
